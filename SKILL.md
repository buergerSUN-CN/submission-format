...
- 表：全框线 sz=4、单元格 9pt、首列左/数据列中、表块居中、满宽；表头加粗；中点小数(`\cdot`→·)；宽表(≥N 列)横向。
- 题注：表上/图下，"Table N./Figure N." 加粗 9pt 前缀。
- References：左对齐(jc=start，与正文 both 不同)、无缩进、11pt、手打编号。
